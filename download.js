...
async function scrollToBottom(page) {
    ...
    const step = 1000;
    const totalSteps = Math.ceil((scrollHeight + 60000) / step);

    const bar = new cliProgress.SingleBar({}, cliProgress.Presets.shades_classic);
    bar.start(totalSteps, 0);

    for (let i = 0; i < scrollHeight + 70000; i += step) {
            ...
            .reduce((acc, div) => {
                ...
                const squares = Array
                    ...
                    .map(t => {
                        ...
                        if (fighter && team?.toLowerCase().includes(academy.toLowerCase()) && cate.includes('NOIRE')) {
